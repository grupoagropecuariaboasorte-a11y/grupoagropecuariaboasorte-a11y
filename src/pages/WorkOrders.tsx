...
export default function WorkOrders({ selectedFarmId, userRole }: WorkOrdersProps) {
  ...
  const refreshList = async () => {
    try {
      ...
      console.error('Erro ao recarregar lista de O.S.:', e);
    }
  };

  const normalizePriority = (p?: string): 'Alta' | 'Média' | 'Baixa' => {
    if (!p) return 'Média';
    const lower = p.toLowerCase();
    if (lower.includes('alt')) return 'Alta';
    if (lower.includes('med') || lower.includes('méd')) return 'Média';
    if (lower.includes('baix')) return 'Baixa';
    return 'Média';
  };

  const normalizeStatus = (s?: string): 'Aberta' | 'Em Andamento' | 'Concluída' | 'Cancelada' => {
    if (!s) return 'Aberta';
    const lower = s.toLowerCase();
    if (lower.includes('andamento') || lower.includes('progresso')) return 'Em Andamento';
    if (lower.includes('conclu') || lower.includes('fechad')) return 'Concluída';
  ...
  const handleSubmitAdd = async (e: React.FormEvent) => {
    ...
    if (!formDescription.trim()) {
      alert('Por favor, descreva o problema ou serviço a ser realizado.');
      return;
    }
    try {
      setSubmitting(true);
      await fleetService.addWorkOrder({
        ...
        status: 'Aberta',
        assigned_to: formAssignedTo.trim() || 'A definir',
        responsible: formAssignedTo.trim() || 'A definir'
      });
      setIsAddOpen(false);
      await refreshList();
      alert('Ordem de serviço (O.S.) emitida manualmente com sucesso!');
    } catch (err: any) {
      console.error('Erro ao abrir OS:', err);
      alert('Erro ao abrir O.S.: ' + (err?.message || err || 'Erro de conexão'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSubmitEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsEditOpen(false);
      await refreshList();
      alert('Ordem de serviço (O.S.) atualizada com sucesso!');
    } catch (err: any) {
      console.error('Erro ao atualizar OS:', err);
      alert('Erro ao atualizar O.S.: ' + (err?.message || err || 'Erro de conexão'));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <Modal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="Abertura de Chamado / Ordem de Serviço Manual (O.S.)">
        <form onSubmit={handleSubmitAdd} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Equipamento com Problema *</label>
              <select
                value={formMachineId}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Nível de Prioridade</label>
              <select
                value={formPriority}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1.5">
              Técnico / Oficina Encarregada <span className="text-slate-400 font-normal">(Opcional)</span>
            </label>
            <input
              type="text"
              placeholder="Ex: Auto Elétrica Diesel, Mecânico João (ou deixe em branco)"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1.5">
              Descrição Técnica do Defeito / Diagnóstico / Serviço a Realizar *
            </label>
            <textarea
              required
              value={formDescription}
              onChange={(e) => setFormDescription(e.target.value)}
              placeholder="Descreva detalhadamente o problema ou serviço de manutenção necessário..."
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-4 py-2 bg-[#1B3022] hover:opacity-90 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-xs cursor-pointer transition-all flex items-center gap-1.5"
            >
              {submitting ? (
                <>
